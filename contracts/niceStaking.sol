...
contract BitcrushNiceStaking is Ownable {
    ...
    constructor (CRUSHToken _crush, uint256 _crushPerBlock, address _reserveAddress, BEP20 _nice) public{
        crush = _crush;
        nice = _nice;
    ...
    function updateProfits() public {
        if(stakingPool.totalShares() == 0)
            return;
        //Todo replace with galatic chef rewards    
        uint256 requestedProfits = 1000000000000000000;//bankroll.transferProfit();
        if(requestedProfits == 0)
            return;
        totalProfitDistributed = totalProfitDistributed.add(requestedProfits);
        uint256 profitCalc = requestedProfits.mul(1e12).div(stakingPool.totalShares());
    ...
    function compoundAll () public  {
        ...
        if(stakingPool.indexesLength() <= stakingPool.autoCompoundLimit() || batchStart.add(autoCompoundLimit) >= stakingPool.indexesLength())
            batchLimit = stakingPool.indexesLength();
        else
            batchLimit = batchStart.add(stakingPool.autoCompoundLimit());

        updateProfits();
        for(uint256 i=batchStart; i < batchLimit; i++){
            UserStaking storage currentUser = stakings[stakingPool.addressIndexes(i)];
            (currentUser.shares, , , , , , , , ) = stakingPool.stakings(stakingPool.addressIndexes(i));
            
            uint256 stakerReward = currentUser.shares.mul(accProfitPerShare).div(1e12).sub(currentUser.profitBaseline);
            currentUser.profitBaseline = currentUser.profitBaseline.add(stakerReward);
            
            
            if(stakerReward > 0){
                
                niceRewards[stakingPool.addressIndexes(i)] = niceRewards[stakingPool.addressIndexes(i)].add(stakerReward);
                //TODO revisit
