...
// VRF
//chainlink
import "@smartcontractkit/contracts/src/v0.8/VRFConsumerBase.sol";
//openzeppelin
    ...
    IBitcrushLottery
{
        ...
        nonReentrant
    {
        ...
        uint256 ticketCost = roundInfo[currentRound].ticketValue.mul(
        ...
        userTotalTickets[msg.sender] = userTotalTickets[msg.sender].add(
            _ticketNumbers.length
        );
        uint256 devCut = getFraction(ticketCost, devTicketCut, PERCENT_BASE);
        ...
        if (_partnerId > 0) {
            ...
            uint256 partnerCut = getFraction(devCut, _p.spread, 100);
        ...
        roundInfo[currentRound].totalTickets = roundInfo[currentRound]
            .totalTickets
            .add(_ticketNumbers.length);
        userRoundTickets[msg.sender][currentRound]
            .totalTickets = userRoundTickets[msg.sender][currentRound]
            .totalTickets
            .add(_ticketNumbers.length);
        ...
        operatorOnly
    {
        ...
        if (_split > 0) partnerSplit[_partnerAddress].set = true;
        ...
        override
    {
        ...
        for (
            uint256 exchange = 0;
            exchange < _ticketNumbers.length;
            exchange++
        ) {
            createTicket(
                msg.sender,
    ...
    function setBonusCoin(
        ...
        require(
            bonusToken.balanceOf(msg.sender) >= _amount,
            "Funds are needed"
        );
    ...
    function crushTheContract() external override onlyOwner {
        ...
        ERC20(0x404460C6A5EdE2D891e8297795264fDe62ADBB75).safeTransfer(
            msg.sender,
        ...
        returns (NewTicket[] memory)
    {
        ...
        for (uint256 i = 0; i < roundReview.totalTickets; i++)
            tickets[i] = userNewTickets[msg.sender][
        ...
        returns (uint8 _match)
    {
        ...
        for (uint8 i = 0; i < 6; i++) {
            if (winnerDigits[i] == luckyDigits[i]) _match = i + 1;
            else break;
        }
    }
    ...
    function calcNextHour() internal {
        ...
        while (tempEnd <= block.timestamp) {
            ...
            if (newIndex == endHours.length) nextDay = false;
        }
        roundEnd = tempEnd;
        ...
        )
    {
        ...
        for (uint8 i = 0; i < 6; i++) {
            ...
            if (matchHolders > 0) {
                ...
                if (matchHolders > 0) {
                    ...
                    roundBonusCoin.bonusMaxPercent = roundBonusCoin
                        .bonusMaxPercent
                        .add(info.distribution[6 - i]);
                }
            } else
                _rollover = _rollover.add(
                    getFraction(
                        info.pool,
        ...
        if (_forClaimer < 10 * ONE__PERCENT) {
            ...
            if (
                _distributed <
                getFraction(
                    info.totalTickets.mul(info.ticketValue),
                    devTicketCut,
                    PERCENT_BASE
                )
            ) {
                _distributed = getFraction(
                    info.totalTickets.mul(info.ticketValue),
                    devTicketCut,
                    PERCENT_BASE
                ).sub(_distributed);
        ...
        if (nonWinners == 0)
            ...
            roundBonusCoin.bonusMaxPercent = roundBonusCoin.bonusMaxPercent.add(
                info.distribution[0]
            );
        if (
            getFraction(info.pool, burnThreshold, PERCENT_BASE) <=
            info.totalTickets.mul(info.ticketValue)
        ) _burn = getFraction(info.pool, burn, PERCENT_BASE);
        else {
            _burn = 0;
            _rollover = _rollover.add(
                getFraction(info.pool, burn, PERCENT_BASE)
            );
        }
