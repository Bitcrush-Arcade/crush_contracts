...
contract( "LotteryTests", ([alice, bob, carol, dev, minter, partner, monkey, bull, bear, claimer]) => {
  ...
  it("test buying tickets", async() => {
    ...
    const ticket2 = 789456
    const ticket3 = 123756
    const ticket4 = 954600
    const ticket5 = 068977
    const ticket6 = 481075
    const monkeyEthBalance = new BgN(await web3.eth.getBalance(monkey))
    await this.lottery.buyTickets([ticket1, ticket2, ticket3, ticket4, ticket5, ticket6,ticket3, ticket4, ticket5, ticket6],0,{from: monkey});
    ...
    await this.lottery.buyTickets([789456],0,{from: alice});
    const aliceEthAfter = new BgN(await web3.eth.getBalance(alice))
    const bobEthBalance = new BgN(await web3.eth.getBalance(bob))
    await this.lottery.buyTickets([123756],0,{from: bob});
    const bobEthAfter = new BgN(await web3.eth.getBalance(bob))

    console.log(`holders ${new BgN(await this.lottery.holders(1,1789456)).toString()}`)
    const monkeyTicket = await this.lottery.userNewTickets(monkey,1)
    console.log(`ticket ${new BgN(monkeyTicket.ticketNumber).toString()}  round ${new BgN(monkeyTicket.round).toString()}`)
